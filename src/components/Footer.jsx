...
function Footer() {
    ...
    return (<footer className="bg-gradient-to-b from-white to-gray-50 border-t border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          ...
          <div className="lg:col-span-2 md:col-span-1">
            ...
            <div className="flex space-x-3">
              {socialLinks.map(function (social) { return (<a key={social.name} href={social.url} target="_blank" rel="noopener noreferrer" className={"w-10 h-10 flex items-center justify-center rounded-xl bg-gray-50 text-gray-600 ".concat(social.color, " transition-all duration-200 shadow-sm hover:shadow-md")} aria-label={"Follow us on ".concat(social.name)}>
                  {social.icon}
                </a>); })}
            </div>
          </div>
          ...
          {Object.entries(footerLinks).map(function (_a) {
            ...
            return (<div key={category}>
              ...
              <ul className="space-y-3">
                {links.map(function (link) { return (<li key={link.name}>
                    <react_router_dom_1.Link to={link.path} className="text-gray-600 hover:text-blue-600 transition-colors duration-200 hover:pl-2 inline-block">
                      {link.name}
