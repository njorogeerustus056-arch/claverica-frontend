...
var Checkbox = function (_a) {
    var label = _a.label, checked = _a.checked, id = _a.id, onChange = _a.onChange, _b = _a.className, className = _b === void 0 ? "" : _b, _c = _a.disabled, disabled = _c === void 0 ? false : _c;
    return (<label className={"flex items-center space-x-3 group cursor-pointer ".concat(disabled ? "cursor-not-allowed opacity-60" : "")}>
      <div className="relative w-5 h-5">
        <input id={id} type="checkbox" className={"w-5 h-5 appearance-none cursor-pointer dark:border-gray-700 border border-gray-300 checked:border-transparent rounded-md checked:bg-brand-500 disabled:opacity-60 \n          ".concat(className)} checked={checked} onChange={function (e) { return onChange(e.target.checked); }} disabled={disabled}/>
