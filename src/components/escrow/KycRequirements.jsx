...
exports.default = KycRequirements;
var lucide_react_1 = require("lucide-react");
function KycRequirements(_a) {
    var amount = _a.amount, serviceType = _a.serviceType;
    var requirements = [
        {
        ...
        {
            icon: <lucide_react_1.AlertCircle className="w-5 h-5"/>,
    ...
    var benefits = [
        {
            icon: <lucide_react_1.Zap className="w-5 h-5 text-yellow-400"/>,
            title: "Higher Limits",
            description: "Unlock transactions up to $50,000"
        },
        {
            icon: <lucide_react_1.Award className="w-5 h-5 text-purple-400"/>,
            title: "Premium Features",
            description: "Access advanced escrow options"
        },
        {
            icon: <lucide_react_1.Shield className="w-5 h-5 text-green-400"/>,
            title: "Enhanced Security",
            description: "Additional fraud protection"
        },
        {
            icon: <lucide_react_1.Globe className="w-5 h-5 text-blue-400"/>,
            title: "Global Access",
        ...
        <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-white/10">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center">
              <lucide_react_1.AlertCircle className="w-6 h-6 text-white"/>
            </div>
            <div>
          ...
          <div className="space-y-4">
            {requirements.map(function (req, index) { return (<div key={index} className={"flex items-start gap-4 p-4 rounded-xl transition-all ".concat(req.required
                ? 'bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/20'
                : 'bg-white/5 border border-white/10')}>
                <div className={"w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ".concat(req.required ? 'bg-blue-500/20' : 'bg-gray-500/20')}>
                  {req.icon}
                </div>
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-white font-medium">{req.title}</p>
                    {req.required ? (<span className="text-xs bg-red-500/20 text-red-300 px-2 py-1 rounded">
                        Required
                      </span>) : (<span className="text-xs bg-gray-500/20 text-gray-300 px-2 py-1 rounded">
                        Optional
                      </span>)}
                  </div>
        ...
        <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 border border-white/10">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-green-500 to-emerald-500 flex items-center justify-center">
              <lucide_react_1.Award className="w-6 h-6 text-white"/>
            </div>
            <div>
          ...
          <div className="space-y-4">
            {benefits.map(function (benefit, index) { return (<div key={index} className="flex items-start gap-4 p-4 bg-gradient-to-r from-green-500/5 to-emerald-500/5 rounded-xl border border-green-500/20 hover:border-green-500/30 transition-all group cursor-pointer">
