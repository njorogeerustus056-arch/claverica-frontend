...
var LoanRecommendation = function (_a) {
    var userProfile = _a.userProfile, loans = _a.loans, onSelectLoan = _a.onSelectLoan;
    // ── scoring engine ──────────────────────────────────────
    var getRecommendedLoans = function () {
        ...
        return loans
            ...
            if (loan.popular)
                score += 25;
            var rate = parseFloat(loan.interest.replace("%", ""));
            score += Math.max(0, 15 - rate) * 2;
            if (loan.fundingTime.includes("Same day") ||
                loan.fundingTime.includes("Instant"))
                score += 20;
            else if (loan.fundingTime.includes("24"))
                score += 15;
            score += loan.features.length * 3;
            score += timeBonus[loan.id] || 0;
            return __assign(__assign({}, loan), { score: score });
        })
            .sort(function (a, b) { return b.score - a.score; })
            .slice(0, 3);
    };
    var recommended = getRecommendedLoans();
    var getReason = function (loan) {
        if (loan.id === "emergency")
            return "Fast funding for urgent needs";
        if (loan.id === "personal")
            return "Flexible for various purposes";
        if (loan.id === "business")
            return "Supports growth & expansion";
        if (loan.id === "education")
            return "Invest in your future";
        return "Best value for your current profile";
    };
    // Match % — highest for #1, descending
    var matchPct = function (idx) { return Math.max(92, 98 - idx * 3); };
    return (<div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-violet-500/10 border border-violet-500/30 flex items-center justify-center">
              <lucide_react_1.Brain className="w-5 h-5 text-violet-400"/>
            </div>
            <div>
          ...
          <div className="flex items-center gap-1.5 bg-violet-500/10 border border-violet-500/30 px-2.5 py-1 rounded-md">
            <lucide_react_1.Sparkles className="w-3 h-3 text-violet-400"/>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {recommended.map(function (loan, idx) {
            var Icon = loan.icon;
            var rank = RANK_COLORS[idx];
            var match = matchPct(idx);
            return (<framer_motion_1.motion.div key={loan.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.08 }} whileHover={{ y: -3 }} onClick={function () { return onSelectLoan(loan.id); }} className="relative bg-slate-800/50 border border-slate-700/60 hover:border-slate-600 rounded-xl p-4 cursor-pointer transition-all duration-250 flex flex-col">
                {/* ── rank badge + score pill ────────────── */}
                <div className="flex items-center justify-between mb-4">
                  <div className={"w-8 h-8 rounded-lg bg-gradient-to-br ".concat(rank.bg, " shadow-sm ").concat(rank.shadow, " flex items-center justify-center")}>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {idx === 0 && <lucide_react_1.Rocket className="w-3.5 h-3.5 text-amber-400"/>}
                  </div>
                </div>

                {/* ── loan info ──────────────────────────── */}
                <div className="flex items-center gap-3 mb-4">
                  <div className={"w-10 h-10 rounded-xl bg-gradient-to-br ".concat(loan.color, " flex items-center justify-center")}>
                ...
                <div className="flex items-start gap-2 bg-slate-800/60 border border-slate-700/40 rounded-lg p-3 mb-4">
                  <lucide_react_1.ThumbsUp className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0 mt-0.5"/>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-2 mb-4">
                  ...
                  <div className="text-center bg-slate-800/50 border border-slate-700/30 rounded-lg py-2">
                    <div className="flex items-center justify-center gap-1">
                      <lucide_react_1.Clock className="w-3 h-3 text-slate-500"/>
                ...
                <div className="mb-4">
                  ...
                  <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
                    <div className="h-full rounded-full bg-gradient-to-r from-emerald-500 to-cyan-500 transition-all duration-500" style={{ width: "".concat(match, "%") }}/>
                  </div>
                </div>
                ...
                <div className="flex gap-2 mt-auto">
                  <button onClick={function (e) {
                    ...
                    Select
                    <lucide_react_1.Target className="w-3 h-3 group-hover:scale-110 transition-transform"/>
                  </button>
                  <button className="w-9 flex items-center justify-center border border-slate-700 hover:border-slate-500 rounded-lg transition-all">
                    <lucide_react_1.TrendingUp className="w-3.5 h-3.5 text-slate-500"/>
                  </button>
                </div>
        ...
        <div className="mt-6 pt-5 border-t border-slate-800 flex items-start gap-3">
          <div className="w-8 h-8 rounded-lg bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center flex-shrink-0">
            <lucide_react_1.Zap className="w-3.5 h-3.5 text-cyan-400"/>
          </div>
          <div>
...
exports.default = LoanRecommendation;
