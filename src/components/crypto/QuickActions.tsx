...
import { Plus, Send, ArrowUpRight, ArrowDownRight, Settings, Shield, Zap, X, ChevronRight, TrendingUp, Sparkles, Lock, Clock } from 'lucide-react';
...
export const QuickActions = ({ onDeposit, onTransfer, onBuy, user, kycThreshold }: QuickActionsProps) => {
  ...
  const handleQuickBuy = (amount: number) => {
    if (!user?.is_verified && amount > kycThreshold) {
      onBuy(amount);
    } else {
      onBuy(amount);
    }
    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className={`group relative w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-600 via-blue-500 to-purple-600 shadow-2xl hover:shadow-3xl transition-all duration-300 flex items-center justify-center ${
            isOpen ? 'scale-95 rotate-45' : 'scale-100 hover:scale-110'
          }`}
        >
          {/* Pulsing ring */}
        ...
        <>
          ...
          <div className="fixed bottom-24 right-6 z-50 w-96 max-h-[80vh] overflow-y-auto bg-white dark:bg-slate-900 rounded-3xl shadow-2xl animate-in slide-in-from-bottom-8 duration-500 border border-slate-200 dark:border-slate-800">
            ...
            <div className="p-6 space-y-6">
              {/* KYC Warning */}
              {!user?.is_verified && (
                <div className="relative overflow-hidden p-5 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-2xl border-2 border-purple-200 dark:border-purple-700/50">
                  {/* Decorative element */}
              ...
              <div>
                  ...
                  {quickAmounts.map((amount) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative px-4 py-3.5 rounded-xl text-sm font-bold transition-all duration-300 ${
                          isSelected
                      ...
                      >
                        {requiresKYC && (
                          <Lock className="absolute top-1 right-1 w-3 h-3 text-purple-400" />
                        )}
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-4 py-3.5 bg-slate-100 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 focus:border-blue-500 dark:focus:border-blue-500 rounded-xl text-slate-900 dark:text-white font-bold outline-none transition-all"
                  ...
                  {quickActions.map((action) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group/action relative p-5 rounded-2xl transition-all duration-300 overflow-hidden ${
                      ...
                      >
                        ...
                        {action.kycRequired && (
                          <div className="absolute top-2 right-2 w-6 h-6 bg-yellow-400 rounded-full flex items-center justify-center shadow-lg">
                            <Lock className="w-3.5 h-3.5 text-yellow-900" strokeWidth={2.5} />
                          </div>
                        )}
