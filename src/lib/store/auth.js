"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};

// src/lib/store/auth.ts - CLEAN VERSION WITH REDUCED LOGS
var zustand_1 = require("zustand");
var middleware_1 = require("zustand/middleware");
var API_URL = import.meta.env.VITE_API_URL || import.meta.env.VITE_API_URL;

// Store creation
var useAuthStore = (0, zustand_1.create)()((0, middleware_1.persist)(function (set, get) { return ({
    user: null,
    tokens: null,
    loading: false,
    isAuthenticated: false,
    syncFromLocalStorage: function () {
        if (typeof window === 'undefined')
            return;
        var localToken = localStorage.getItem('token');
        var localRefresh = localStorage.getItem('refresh_token');
        if (localToken && localRefresh) {
            set({
                tokens: { access: localToken, refresh: localRefresh }
            });
        }
    },
    login: function (email, password) { return __awaiter(void 0, void 0, void 0, function () {
        var response, error, data, error_1;
        var _a, _b;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    set({ loading: true });
                    _c.label = 1;
                case 1:
                    _c.trys.push([1, 6, , 7]);
                    return [4 /*yield*/, fetch("".concat(API_URL, "/api/accounts/login/"), {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ email: email, password: password }),
                        })];
                case 2:
                    response = _c.sent();
                    if (!!response.ok) return [3 /*break*/, 4];
                    return [4 /*yield*/, response.json()];
                case 3:
                    error = _c.sent();
                    throw new Error(error.message || error.detail || 'Login failed');
                case 4: return [4 /*yield*/, response.json()];
                case 5:
                    data = _c.sent();
                    if ((_a = data.tokens) === null || _a === void 0 ? void 0 : _a.access) {
                        localStorage.setItem('token', data.tokens.access);
                    }
                    if ((_b = data.tokens) === null || _b === void 0 ? void 0 : _b.refresh) {
                        localStorage.setItem('refresh_token', data.tokens.refresh);
                    }
                    set({
                        user: data.account,
                        tokens: data.tokens,
                        isAuthenticated: true,
                        loading: false,
                    });
                    return [2 /*return*/, true];
                case 6:
                    error_1 = _c.sent();
                    console.error('Login error:', error_1);
                    set({ loading: false });
                    return [2 /*return*/, false];
                case 7: return [2 /*return*/];
            }
        });
    }); },
    logout: function () {
        localStorage.removeItem('token');
        localStorage.removeItem('refresh_token');
        set({
            user: null,
            tokens: null,
            isAuthenticated: false,
        });
    },
    clearAuth: function () {
        localStorage.removeItem('token');
        localStorage.removeItem('refresh_token');
        set({
            user: null,
            tokens: null,
            isAuthenticated: false,
        });
    },
    refreshToken: function () { return __awaiter(void 0, void 0, void 0, function () {
        var tokens, response, data, _a;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    tokens = get().tokens;
                    if (!(tokens === null || tokens === void 0 ? void 0 : tokens.refresh))
                        return [2 /*return*/, false];
                    _b.label = 1;
                case 1:
                    _b.trys.push([1, 4, , 5]);
                    return [4 /*yield*/, fetch("".concat(API_URL, "/api/accounts/refresh/"), {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ refresh: tokens.refresh }),
                        })];
                case 2:
                    response = _b.sent();
                    if (!response.ok)
                        return [2 /*return*/, false];
                    return [4 /*yield*/, response.json()];
                case 3:
                    data = _b.sent();
                    if (data.access) {
                        localStorage.setItem('token', data.access);
                    }
                    set({
                        tokens: __assign(__assign({}, tokens), { access: data.access }),
                    });
                    return [2 /*return*/, true];
                case 4:
                    _a = _b.sent();
                    get().clearAuth();
                    return [2 /*return*/, false];
                case 5: return [2 /*return*/];
            }
        });
    }); },
    setLoading: function (loading) { return set({ loading: loading }); },
    checkAuth: function () {
        get().syncFromLocalStorage();
        var _a = get(), tokens = _a.tokens, user = _a.user;
        if ((tokens === null || tokens === void 0 ? void 0 : tokens.access) && user) {
            set({ isAuthenticated: true });
            return;
        }
        if ((tokens === null || tokens === void 0 ? void 0 : tokens.access) && !user) {
            set({ isAuthenticated: false });
            return;
        }
        set({ isAuthenticated: false });
    },
    verifyToken: function () { return __awaiter(void 0, void 0, void 0, function () {
        var _a, tokens, user, response, userData, refreshed, error_2;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    get().syncFromLocalStorage();
                    _a = get(), tokens = _a.tokens, user = _a.user;
                    if (!(tokens === null || tokens === void 0 ? void 0 : tokens.access)) {
                        return [2 /*return*/, false];
                    }
                    _b.label = 1;
                case 1:
                    _b.trys.push([1, 11, , 12]);
                    return [4 /*yield*/, fetch("".concat(API_URL, "/api/users/me/"), {
                            headers: {
                                Authorization: "Bearer ".concat(tokens.access),
                                'Content-Type': 'application/json',
                            },
                        })];
                case 2:
                    response = _b.sent();
                    if (!response.ok) return [3 /*break*/, 4];
                    return [4 /*yield*/, response.json()];
                case 3:
                    userData = _b.sent();
                    set({
                        isAuthenticated: true,
                        user: userData
                    });
                    return [2 /*return*/, true];
                case 4:
                    if (!(response.status === 401)) return [3 /*break*/, 9];
                    return [4 /*yield*/, get().refreshToken()];
                case 5:
                    refreshed = _b.sent();
                    if (!refreshed) return [3 /*break*/, 7];
                    return [4 /*yield*/, get().verifyToken()];
                case 6: return [2 /*return*/, _b.sent()];
                case 7:
                    get().logout();
                    return [2 /*return*/, false];
                case 8: return [3 /*break*/, 10];
                case 9: return [2 /*return*/, false];
                case 10: return [3 /*break*/, 12];
                case 11:
                    error_2 = _b.sent();
                    console.error('Token verification error:', error_2);
                    return [2 /*return*/, false];
                case 12: return [2 /*return*/];
            }
        });
    }); },
    updateUser: function (userData) {
        set(function (state) { return ({
            user: state.user ? __assign(__assign({}, state.user), userData) : null,
        }); });
    },
}); }, {
    name: 'auth-storage',
    partialize: function (state) { return ({
        user: state.user,
        tokens: state.tokens,
    }); },
    migrate: function (persistedState, version) {
        if (typeof window !== 'undefined') {
            var localToken = localStorage.getItem('token');
            var localRefresh = localStorage.getItem('refresh_token');
            if (localToken && localRefresh) {
                if (!persistedState.tokens || !persistedState.tokens.access) {
                    persistedState.tokens = {
                        access: localToken,
                        refresh: localRefresh
                    };
                }
            }
        }
        return persistedState;
    }
}));

// FIX: Proper ES module export (removed CommonJS exports)
export { useAuthStore };
